...
function formatDate(date) {
  ...
  const dateElem = new Date(date);
  return new Intl.DateTimeFormat('en-US', options).format(dateElem);
}
...
function getCountWeekendsInMonth(month, year) {
  const dateElem = new Date(year, month, 0);
  const daysInMonth = dateElem.getDate();

  let weekends = 0;

  for (let i = 1; i <= daysInMonth; i += 1) {
    const dayOfMonth = new Date(dateElem);
...
function getWeekNumberByDate(date) {
  ...
  const diffInDays = (date - firstDayOfYear) / oneDayInMs;
  const weeks = Math.ceil((diffInDays + firstDayOfYear.getDay()) / 7);

  return firstDayOfYear.getDay() === 0 ? weeks + 1 : weeks;
}
...
function getNextFridayThe13th(date) {
  ...
  if (dateElem.getDate() !== 13) {
    dateElem.setDate(13);
  }

  while (dateElem.getDay() !== 5) {
